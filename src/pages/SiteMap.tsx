...
const EXPECTED_PAGES = {
  ...
  conversion: [
    ...
    { url: '/case-studies', name: 'Case Studies Index' },
  ],
  caseStudies: [
    ...
    { url: '/case-studies/healthcare-derby', name: 'Healthcare - Derby' },
    { url: '/case-studies/hotel-leeds', name: 'Hotel - Leeds' },
    { url: '/case-studies/student-campus-nottingham', name: 'Student Campus - Nottingham' },
    { url: '/case-studies/data-room-london', name: 'Data Room - London' },
    { url: '/case-studies/distribution-chesterfield', name: 'Distribution - Chesterfield' },
    { url: '/case-studies/emergency-lighting-lincoln', name: 'Emergency Lighting - Lincoln' },
  ],
};

// Generate compliance location pages matrix
const SERVICES = [
  { slug: 'fire', name: 'Fire Safety' },
  { slug: 'electrical', name: 'Electrical Compliance' },
  { slug: 'emergency-lighting', name: 'Emergency Lighting' },
  { slug: 'water', name: 'Water Hygiene' },
  { slug: 'gas', name: 'Gas Safety' },
  { slug: 'hvac', name: 'HVAC Compliance' },
  { slug: 'ppm', name: 'PPM' },
];
...
const SiteMap = () => {
  ...
  const complianceLocationPages = useMemo(() => {
    const pages: { service: string; location: string; url: string; status: string }[] = [];
  ...
  const filteredPages = useMemo(() => {
    ...
    const allPages = [
      ...EXPECTED_PAGES.complianceHubs,
      ...EXPECTED_PAGES.sectors,
      ...EXPECTED_PAGES.conversion,
      ...EXPECTED_PAGES.caseStudies,
      ...complianceLocationPages.map(p => ({ url: p.url, name: `${p.service} - ${p.location}` })),
    ];
  ...
  const exportCSV = () => {
    const allPages = [
      ...EXPECTED_PAGES.complianceHubs.map(p => ({ ...p, category: 'Compliance Hub', status: checkPageStatus(p.url) })),
      ...EXPECTED_PAGES.sectors.map(p => ({ ...p, category: 'Sector', status: checkPageStatus(p.url) })),
      ...EXPECTED_PAGES.conversion.map(p => ({ ...p, category: 'Conversion', status: checkPageStatus(p.url) })),
      ...EXPECTED_PAGES.caseStudies.map(p => ({ ...p, category: 'Case Study', status: checkPageStatus(p.url) })),
    ...
    const csv = [
      ['Category', 'Page Name', 'URL', 'Status'],
      ...allPages.map(page => [page.category, page.name, page.url, page.status])
    ...
    <>
      ...
      <div className="min-h-screen bg-background py-12">
        <div className="container mx-auto px-4 max-w-7xl">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-foreground mb-4">Site Map & URL Registry</h1>
            ...
            <div className="flex gap-4 items-center mb-6">
              <div className="relative flex-1 max-w-md">
                ...
                <Input
                  type="text"
                  placeholder="Search URLs or page names..."
                  value={searchQuery}
              ...
              <CardContent>
                <div className="space-y-2">
                  {filteredPages.map((page) => {
                    ...
                    return (
                      <div key={page.url} className="flex items-center justify-between p-3 border border-border rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border">
                          ...
                          {LOCATIONS.map(loc => (
                            <th key={loc.slug} className="text-center p-2 font-semibold">{loc.name}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {SERVICES.map(service => (
                          <tr key={service.slug} className="border-b border-border hover:bg-muted/50">
                            <td className="p-2 font-medium">{service.name}</td>
                            {LOCATIONS.map(location => {
                              const page = complianceLocationPages.find(
                                p => p.service === service.name && p.location === location.name
                              );
                              return (
                                <td key={location.slug} className="text-center p-2">
                                  {page && (
                                    <Link to={page.url} className="inline-block">
