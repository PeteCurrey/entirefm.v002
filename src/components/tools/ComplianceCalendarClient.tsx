      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="p-8">
          ...
          <div className="flex flex-col items-center gap-6 border-t border-border pt-8 text-center px-4">
            ...
            <button onClick={() => setState("calendar")} className="bg-primary text-white px-10 py-4 rounded-xl font-bold uppercase tracking-widest text-sm hover:bg-primary/90 transition-all shadow-xl">
        ...
        <div className="lg:col-span-3">
          <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4">
            {MONTHS.map((month, idx) => (
              <div key={month} className={`bg-white border rounded-xl overflow-hidden shadow-sm transition-all flex flex-col min-h-[160px] ${idx === new Date().getMonth() ? "border-primary ring-1 ring-primary/20 bg-primary/[0.02]" : "border-border"}`}>
                <div className={`px-4 py-2 border-b text-[11px] font-bold uppercase tracking-[0.15em] flex justify-between items-center ${idx === new Date().getMonth() ? "bg-primary text-white" : "bg-muted text-muted-foreground border-border"}`}>
                  {month}
                  {idx === new Date().getMonth() && <span className="bg-white/20 px-1.5 py-0.5 rounded text-[8px]">Current</span>}
                </div>
                <div className="p-3 space-y-1.5 flex-1 overflow-y-auto max-h-[140px] custom-scrollbar">
                  {monthlySchedule[month].length > 0 ? (
                    monthlySchedule[month].map((event) => (
                      <button key={event.id} onClick={() => setSelectedEvent(event)} className={`w-full text-left p-2 rounded text-[10px] leading-tight transition-all border group ${event.statutory ? "bg-red-50/50 border-red-100 text-red-900" : "bg-blue-50/50 border-blue-100 text-blue-900"}`}>
                        <div className="font-bold flex justify-between gap-1 items-start">
                          <span className="truncate">{event.title}</span>
                          {event.statutory && <ShieldCheck className="w-2.5 h-2.5 shrink-0 text-red-500" />}
                        </div>
                        <div className="opacity-60 flex items-center gap-1 mt-0.5 uppercase tracking-tighter">
                          <Clock className="w-2 h-2 shrink-0" /> {event.frequency}
                        </div>
                      </button>
                    ))
                  ) : (
